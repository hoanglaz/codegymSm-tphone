<form method="post" action="{{ route('homes.update',$homes[11]->id) }}"> @csrf @method('PUT')
    <div class="form-group row">
        <label class="col-sm-2 col-form-label">{{__('List product 1st')}}</label>
        <div class="col-sm-8">
            <select name="home[product1]" class="form-control">
                @foreach($categories as $key =>$val)
                    <option @if($val->id == $homes[11]->data) selected @endif value="{{$val->id}}">{{$val->name}}</option>
                @endforeach
            </select>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <select name="home[product2]" class="form-control">
                @foreach($categories as $key =>$val)
                    <option @if($val->id == $homes[12]->data) selected @endif value="{{$val->id}}">{{$val->name}}</option>
                @endforeach
            </select>
            <span class="messages"></span>
        </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <select name="home[product3]" class="form-control">
                @foreach($categories as $key =>$val)
                    <option @if($val->id == $homes[13]->data) selected @endif value="{{$val->id}}">{{$val->name}}</option>
                @endforeach
            </select>
            <span class="messages"></span>
        </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <select name="home[product4]" class="form-control">
                @foreach($categories as $key =>$val)
                    <option @if($val->id == $homes[14]->data) selected @endif value="{{$val->id}}">{{$val->name}}</option>
                @endforeach
            </select>
            <span class="messages"></span>
        </div>
