        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-header-right">
                    <a href="{{ route('lessons.create') }}" class="btn btn-primary pl-1"><i class="ti-plus ml-0 mr-1"></i>{{__('Add lesson')}}</a>
                    ...
                    <table id="simpletable" class="table table-striped table-bordered">
                        ...
                        @foreach($lessons as $key => $val)
                            <tr>
                                ...
                                <td><a href="{{ route('web.lesson',[$val->url,$val->id]) }}" target="_blank">{{ $val->title }}</a></td>
                                ...
                                <td>
                                    <button class="btn-primary float-left"><a href="{{ route('lessons.edit',$val->id) }}" ><i class="ti-pencil-alt"></i></a></button>
                                    <form action="{{route('lessons.destroy',[$val->id])}}" method="POST" class="float-left">
                                         @method('DELETE')
                                         @csrf
